...
func (router RouterGroup) BigModelRouter() {
	...
	// 角色相关
	{
		...
		router.GET("big_model/roles_history", middleware.JwtAuth(), app.RoleUserHistoryListView) // 角色用户历史列表
	...
	// 对话相关
	{
		...
		router.DELETE("big_model/chat", middleware.JwtAdmin(), app.ChatRemoveView)        // 管理员删除对话
	}
